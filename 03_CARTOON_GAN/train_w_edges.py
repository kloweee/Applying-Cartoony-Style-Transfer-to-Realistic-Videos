...
def initialization(disc_R, disc_C, gen_C, gen_R, loader, opt_disc, opt_gen, l1, mse, d_scaler, g_scaler):
    ...
    for idx, (cartoon, real) in enumerate(loop):
...
#         smooth_cartoon = smooth(cartoon)
        ...
        with torch.amp.autocast("cuda"):
            ...
            # ADVERSARIAL LOSS HERE -> INCORPORATE SMOOTHED EDGES
            # run disc on real and fake reals
...
def main():
    ...
    dataset = CartoonRealDataset(
        root_real = config.TRAIN_DIR+"/real", root_cartoon = config.TRAIN_DIR+"/cartoon/disney_people_processed", transform=config.transforms,
    )
